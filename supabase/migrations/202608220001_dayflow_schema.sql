...
create policy "profile self update" on public.profiles for update using (id = auth.uid() or public.is_hr());
...
create policy "attendance self update" on public.attendance_records for update using (profile_id = auth.uid() or public.is_hr());
