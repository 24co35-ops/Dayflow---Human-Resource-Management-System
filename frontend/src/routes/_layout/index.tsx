...
function Dashboard() {
  ...
  const usedPaidDays = sortedLeaves.filter((leave) => leave.name === "Arjun Singh" && leave.type === "Paid time off" && leave.status === "approved").reduce((total, leave) => total + leave.days, 0)
  ...
  const effectiveCheckedIn = dayflowApiEnabled
    ...
    : checkedIn
  const dashboardPeopleCount = dayflow.dashboard.data?.employee_count ?? 25
  const dashboardPresentCount = dayflow.dashboard.data?.present_today ?? (effectiveCheckedIn ? 1 : 0)
  const dashboardAttendanceRate = dashboardPeopleCount > 0 ? Math.round((dashboardPresentCount / dashboardPeopleCount) * 100) : 0
  ...
  const submitLeave = async (draft: LeaveDraft) => {
    if (dayflowApiEnabled) {
      try {
        await dayflow.createLeave.mutateAsync(leaveInputFromDraft(draft))
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label={role === "hr" ? "People present" : "Today’s status"} value={role === "hr" ? `${dashboardAttendanceRate}%` : effectiveCheckedIn ? "Checked in" : "Not checked in"} detail={role === "hr" ? `${dashboardPresentCount} of ${dashboardPeopleCount} checked in` : effectiveCheckedIn ? "Since 09:18 AM · on track" : "Start your workday in one tap"} icon={<CalendarCheck className="size-5" />} tone="lime" action={role === "employee" ? { label: effectiveCheckedIn ? "Check out" : "Check in", onClick: toggleAttendance } : undefined} />
        <MetricCard label={role === "hr" ? "Pending approvals" : "Leave balance"} value={role === "hr" ? String(pendingCount).padStart(2, "0") : `${Math.max(0, 14 - usedPaidDays)} days`} detail={role === "hr" ? "Need your attention today" : `${usedPaidDays} days used this year`} icon={<Clock3 className="size-5" />} tone="peach" action={role === "employee" ? { label: "Request time off", onClick: () => setView("Leave & time off") } : { label: "Review now", onClick: () => setView("Leave & time off") }} />
      ...
      {view === "Overview" && <SaasOverview role={role} checkedIn={effectiveCheckedIn} apiEnabled={dayflowApiEnabled} leaves={sortedLeaves} approvedCount={approvedCount} activity={dayflow.activity.data} dashboard={dayflow.dashboard.data} setView={selectView} updateLeave={updateLeave} />}
