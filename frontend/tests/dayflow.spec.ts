...
test.describe("Dayflow workspace", () => {
  test("employee can draft and confirm a leave request through Flow", async ({
    page,
  }) => {
    await page.goto("/")
    await expect(
      page.getByRole("heading", { name: /Good morning, Arjun/i }),
    ).toBeVisible()
    ...
    await expect(
      page.getByText(/draft a 2-day Sick Leave request/i),
    ).toBeVisible()
  ...
  test("role switch keeps shell identity and navigation context aligned", async ({
    page,
  }) => {
    await page.goto("/")
    await page.getByRole("button", { name: "HR view" }).click()
    await expect(
      page.getByRole("heading", { name: /Good morning, Ashwith/i }),
    ).toBeVisible()
